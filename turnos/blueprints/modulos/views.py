...
def list_active():
    ...
    modulos_activos = Modulo.query.filter(Modulo.estatus == "A").order_by(Modulo.nombre).limit(100).all()
...
def list_inactive():
    ...
    modulos_inactivos = Modulo.query.filter(Modulo.estatus == "B").order_by(Modulo.nombre).limit(100).all()
